...
async function makeDefinitionsCode() {
  let defs = [
    ...
    removeLocalImportsExports(
      (await readFile(path.join(srcPath, 'exported-definitions.d.ts'), 'utf-8')).trim()
    ),
    '// -- Driver definitions --',
    removeLocalImportsExports(
      (await readFile(path.join(srcPath, 'driver-definitions.d.ts'), 'utf-8')).trim()
    ),
